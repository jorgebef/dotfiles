...
function session_manager.load_state(window, pane, name)
  -- TODO: Implement
  ...
  -- local handle = io.popen("wezterm cli list --format json | jq -r 'unique_by(.workspace) | .[] | .workspace'")
  -- for w in handle:lines() do
  --   if name == w then
  --     return
  --   end
  -- end
  ...
  local file_path = wezterm.home_dir .. "/.config/wezterm/wezterm-session-manager/wezterm_state_" .. name .. ".json"

  local workspace_data = load_from_json_file(file_path)
  if not workspace_data then
    window:toast_notification("WezTerm", "Workspace state file not found for workspace: " .. name, nil, 4000)
    return
  end

  if recreate_workspace(window, workspace_data) then
    window:toast_notification("WezTerm", "Workspace state loaded for workspace: " .. name, nil, 4000)
  else
    window:toast_notification("WezTerm", "Workspace state loading failed for workspace: " .. name, nil, 4000)
  end
end
